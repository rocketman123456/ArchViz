...
namespace ArchViz
{
    enum class RENDER_IMAGE_TYPE : uint8_t
    {
        RENDER_IMAGE_TYPE_UNKNOWM = 0,
    ...
    public:
        ...
        RENDER_IMAGE_TYPE m_type {RENDER_IMAGE_TYPE::RENDER_IMAGE_TYPE_UNKNOWM};
...
template<>
struct std::hash<ArchViz::MeshSourceDesc>
{
    size_t operator()(const ArchViz::MeshSourceDesc& rhs) const noexcept { return rhs.getHashValue(); }
};

template<>
struct std::hash<ArchViz::MaterialSourceDesc>
{
    size_t operator()(const ArchViz::MaterialSourceDesc& rhs) const noexcept { return rhs.getHashValue(); }
};
