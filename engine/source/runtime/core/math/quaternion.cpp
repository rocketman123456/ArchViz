...
namespace ArchViz
{
    ...
    Vector3 Quaternion::rotatePoint(const Vector3& rhs) const
    {
        Quaternion vector(0.0f, rhs.x, rhs.y, rhs.z);
        Quaternion final = *this * vector * inverse();
        return Vector3(final.x, final.y, final.z);
    }

    Matrix3x3 Quaternion::rotateMatrix(const Matrix3x3& rhs) const
    {
        Matrix3x3 mat;
        mat.m_rows[0] = rotatePoint(rhs.m_rows[0]);
        mat.m_rows[1] = rotatePoint(rhs.m_rows[1]);
        mat.m_rows[2] = rotatePoint(rhs.m_rows[2]);
        return mat;
    }
